const API_URL = 'https://script.google.com/macros/s/AKfycbx_0jVkZqmj_3kSvhyeMH1zhokw35qDKanx2U94sapMyy0iK8Kok57YNwyhA6bz5zaQhA/exec';
...
function mostrarPopover(e, uf) {
    ...
    if (listaTransportadoras.length > 0) {
        ...
        listaTransportadoras.forEach(t => {
            const linkContato = `<a href="${t.Contato}" target="_blank" class="popover-link">Fazer cotação</a>`;
            html += `<li> 
                        Transportadora: <strong>${t.Transportadora}</strong><br>
                        Carga Máx: <strong>${t.Carga_Max}</strong> | ${linkContato}
                    </li>`;
        });
        html += '</ul>';
    } else {
